...
-- Learn how to perform multiple regressions
...
- Most of the derivations are easier as well.
...
The error terms $\mathbf{e}$ are represented as a column vector $\mathbf{e}$ of dimension $n \times 1$:
...
## Predictors as Functions of Other Predictors
...
In the population, the formula for the slope is analog to the sample estimator: $\beta_1^* = \frac{\mathbb{Cov}[X_1, Y]}{\mathbb{Var}[X_1]}$
...
```{r se-robusto toy fit, message=FALSE, warning=FALSE}
df %>%
...
- There is no theoretical definition of what is an outlier. It is a mattter of subjective judgment.

- Many tests and diagnostics has been proposed over the yeras (DFITS, Cook’s Distance, and Welsch Distance)

- However, as Hansen says in his econometrics textbook, "they are not based on statistical theory" (p.85).

- My recommended approach is to thjink about how the outlier changes the regression estimate.

- DFBETA measures how much the regression coefficient changes when observation $i$ is omitted. 

- The ratio of these changes to the coefficient’s standard error is called its DFBETA

---

# Example of outlier

<img src="outlier_plot.png" alt="plot with influential observation" style="width:50%; height:50%;">
...
# Outlier Diagnostics
```{r outliers data, message=FALSE, warning=FALSE, echo=FALSE}
set.seed(2345)
x <- runif(25, 1, 10)
y <- rnorm(25, x +2, 2)
x[26] <- 9
...
data <- data.frame(x=x,
                   y=y,
                   group = c(rep("open", 25), "filled"))

# Data without the 26th observation
data_no26 <- data[1:25, ]
fit <- lm(y ~ x, data)
```
