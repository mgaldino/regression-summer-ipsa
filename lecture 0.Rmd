...
- **Objective of the Lecture:** Define and explain the concept of the Conditional Expectation Function (CEF) and its relevance for Linear Regression.
...
1. Connection via Ordinary Least Squares

- In a simple linear regression model: \(Y = \alpha + \beta*X + e\).
- This represents the population regression line.
- The aim is to find \(\alpha\) and \(\beta\) that minimize the sum of squared errors.
- We can think of it as an approximation to the CEF.
...
- See "Mostly Harmless Econometrics", chatp. 3, for detailed explanation.
