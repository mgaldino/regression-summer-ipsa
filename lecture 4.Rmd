...
The residuals will always have an expected value of zero, conditional on the predictors. Formally, $\mathbb{E}[\hat{e}|X=x] = 0$.
...
We will try to check as many linear regression assumptions as possible
...
Since the residuals will have $\mathbb{E}[\hat{e}|X=x] = 0$, it means that for each $x_i$, the residuals should have an average of zero. 
...
df <- data.frame(residuals_sq = residuals(fit)^2, predictor = df_sim$x)
df %>%
  ggplot(aes(x=predictor, y = residuals_sq)) + geom_point() +   geom_smooth(method="lm", se=F) + geom_smooth(se=F) 
```
...
fit_correct <-lm(y ~ x + x_sq, df_sim)
df <- data.frame(residuals_sq = residuals(fit_correct)^2, predictor = df_sim$x)
df %>%
  ggplot(aes(x=predictor, y = residuals_sq)) + geom_point() +   geom_smooth(method="lm", se=F) + geom_smooth(se=F) 
```
...
df <- data.frame(residuals_sq = residuals(fit2)^2, predictor = df_sim2$x)
df %>%
  ggplot(aes(x=predictor, y = residuals_sq)) + geom_point() +   geom_smooth(method="lm", se=F) + geom_smooth(se=F) 
```
...
fit_correct2 <-lm(y ~ x + x1, df_sim2)
df <- data.frame(residuals_sq = residuals(fit_correct2)^2, predictor = df_sim2$x)
df %>%
  ggplot(aes(x=predictor, y = residuals_sq)) + geom_point() +   geom_smooth(method="lm", se=F) + geom_smooth(se=F) 
```
...
df <- data.frame(residuals_sq = residuals(fit3)^2, predictor = df_sim3$x)
df %>%
  ggplot(aes(x=predictor, y = residuals_sq)) + geom_point() +geom_smooth(method="lm", se=F) + geom_smooth(se=F) 
```
...
fit_correct3 <-lm(y ~ log_x, df_sim3)
df <- data.frame(residuals_sq = residuals(fit_correct3)^2, predictor = df_sim3$x)
df %>%
  ggplot(aes(x=predictor, y = residuals_sq)) + geom_point() +   geom_smooth(method="lm", se=F) + geom_smooth(se=F) 
```
...
df <- data.frame(residuals_sq = residuals(fit4)^2, predictor = df_sim4$x)
df %>%
  ggplot(aes(x=predictor, y = residuals_sq)) + geom_point() +geom_smooth(method="lm", se=F) + geom_smooth(se=F) 
```
